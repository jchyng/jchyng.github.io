...
const frag = /* glsl */ `
  ...
  vec4 draw(vec2 uv, float hue, float iTime, float noiseScale, vec3 backgroundColor) {
    ...
    float n1 = snoise3(vec3(uv * noiseScale * 0.8, iTime * 0.5 + 100.0)) * 0.5 + 0.5;
    float r1 = mix(0.3, 0.5, n1);
    float v1 = light2(0.8, 20.0, distance(uv, vec2(cos(iTime * 0.7), sin(iTime * 0.7)) * r1));

    float n2 = snoise3(vec3(uv * noiseScale * 1.2, iTime * 0.5 + 200.0)) * 0.5 + 0.5;
    float r2 = mix(0.4, 0.6, n2);
    float v2 = light2(0.7, 15.0, distance(uv, vec2(cos(iTime * -0.8 + 2.0), sin(iTime * -0.8 + 2.0)) * r2));

    vec3 finalColor = color1 * v0 * innerFade + color2 * v1 + color3 * v2;
  ...
  void main() {
    ...
    vec4 orbColor = draw(rotUv, hue, iTime, noiseScale, backgroundColor);
    vec3 finalColor = mix(backgroundColor, orbColor.rgb, orbColor.a * hoverIntensity * (1.0 + hover));
...
export default function Orb({
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const uvX = ((x - centerX) / s) * 2.0;
      const uvY = ((y - centerY) / s) * 2.0;
